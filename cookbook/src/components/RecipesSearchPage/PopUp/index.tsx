...
export default function PopUpRecipeDetailed(props: PopUpRecipeDetailedProps): JSX.Element {
  const { openDetailedInfo, cardInfo } = props;
  const { image, description, name, author, likes, comments, directions, ingredients } = cardInfo;

  function closePopUp(e: React.MouseEvent) {
    const target = e.target as HTMLElement;
    if (target.classList.contains('overlay') || target.classList.contains('overlay__btn')) {
          ...
          <div className="pop-up__recipe">
            ...
            <div className="pop-up-sections">
              <div className="pop-up__section top">
                ...
                <button className="pop-up__btn">+</button>
              </div>
              ...
              <div className="pop-up__section information">
                <div className="pop-up__section directions">
                  ...
                  <ul className="directions-list">
                    {directions.map((el) => (
                      <li><span>{`Step ${directions.indexOf(el) + 1}`}</span>: {el}</li>
                    ))}
                  </ul>
                </div>
