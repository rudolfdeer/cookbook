...
export const modifyRecipe = (recipeId: number, data: RecipeRequestBody, userId: number) => async (dispatch: Dispatch): Promise<void> => {
  await recipeApi.updateRecipe(recipeId, data);
  const recipes = await recipeApi.getAllRecipes();
  dispatch(recipeActions.update(recipes, userId));
};

export const updateRecipesImage = (recipeId: number, data: FormData, userId: number) => async (dispatch: Dispatch): Promise<void> => {
